...
def load_data(filename):
    ...
    with open(filename) as csvfile:
        ...
        for row in reader:
            ...
            entry.append(int(month[row["Month"]]))
            ...
            entry.append(1 if row["VisitorType"] == "Returning_Visitor" else 0)
            entry.append(1 if row["Weekend"] == "TRUE" else 0)
            
            evidence.append(entry)
            label.append(0 if row["Revenue"] == "FALSE" else 1)
...
def evaluate(labels, predictions):
    ...
    """
    arr = confusion_matrix(labels,predictions)
    tn = arr[0][0]
    fn= arr[1][0]
    fp = arr[0][1]
    tp = arr[1][1]
